...
addActionHandler('loadViewportMessages', (global, actions, payload): ActionReturnType => {
  ...
  if (!viewportIds || !viewportIds.length || direction === LoadMoreDirection.Around) {
    ...
    const isBotAccount = global.currentUserId
      && selectUser(global, global.currentUserId)?.type === 'userTypeBot';
    const shouldSkipRemoteLoad = Boolean(isBotAccount && historyIds.length > 0 && !forceLastSlice);

    if (areSomeLocal) {
      global = safeReplaceViewportIds(global, chatId, threadId, newViewportIds, tabId);
    }

    if (!areAllLocal && !shouldSkipRemoteLoad) {
      onTickEnd(() => {
...
addActionHandler('syncBotChatMessages', async (global, actions, payload): Promise<void> => {
  const { chatId } = payload;

  if (!global.currentUserId || selectUser(global, global.currentUserId)?.type !== 'userTypeBot') {
    return;
  }
  ...
  if (chats?.length) {
    chats.forEach((item) => {
      global = updateChat(global, item.id, item);
    });
  }

  if (!messages.length) {
    if (count === 0) {
      global = updateThreadInfoMessagesCount(global, chatId, threadId, 0);
      setGlobal(global);
    }
    return;
  }
  ...
  const ids = messages.map(({ id }) => id).sort((a, b) => a - b);

  const shouldRefreshViewportByTab = new Map<number, boolean>();
  Object.values(global.byTabId).forEach(({ id: currentTabId }) => {
    const messageList = selectCurrentMessageList(global, currentTabId);
    if (messageList?.chatId !== chatId || messageList.type !== 'thread') {
      return;
    }

    const viewportIds = selectViewportIds(global, chatId, threadId, currentTabId);
    const listedIds = selectListedIds(global, chatId, threadId);
    const wasAtBottom = !viewportIds?.length || !listedIds?.length
      || viewportIds[viewportIds.length - 1] === listedIds[listedIds.length - 1];

    shouldRefreshViewportByTab.set(
      currentTabId,
      wasAtBottom || Boolean(selectIsViewportNewest(global, chatId, threadId, currentTabId)),
    );
  });

  global = addChatMessagesById(global, chatId, byId);
  global = updateListedIds(global, chatId, threadId, ids);
  global = updateThreadInfoMessagesCount(global, chatId, threadId, count ?? ids.length);

  shouldRefreshViewportByTab.forEach((shouldRefresh, currentTabId) => {
    if (!shouldRefresh) {
      return;
    }

    const sliceStart = Math.max(0, ids.length - MESSAGE_LIST_SLICE);
    const newViewportIds = ids.slice(sliceStart);
    global = safeReplaceViewportIds(global, chatId, threadId, newViewportIds, currentTabId);
  });

  const lastMessage = messages[messages.length - 1];
  global = updateChatLastMessageId(global, chatId, lastMessage.id);
...
async function loadWithBudget<T extends GlobalState>(
  ...
  const listedIds = selectListedIds(currentGlobal, chat.id, threadId);
  const threadInfo = selectThreadInfo(currentGlobal, chat.id, threadId);
  const isBotAccount = currentGlobal.currentUserId
    && selectUser(currentGlobal, currentGlobal.currentUserId)?.type === 'userTypeBot';
  const isBotHistoryEnd = Boolean(isBotAccount && (
    threadInfo?.messagesCount === 0
    || (listedIds?.length && (
      (direction === LoadMoreDirection.Backwards && offsetId === listedIds[0])
      || (direction === LoadMoreDirection.Forwards && offsetId === listedIds[listedIds.length - 1])
    ))
  ));
  const shouldLoadRemote = !areAllLocal && !isBotHistoryEnd;
...
async function loadViewportMessages<T extends GlobalState>(
  ...
  const isHistoryFullyLoaded = 'isEndOfHistory' in result && result.isEndOfHistory;
  if (isHistoryFullyLoaded && !messages.length) {
    global = updateThreadInfoMessagesCount(global, chat.id, threadId, count ?? 0);
...
function getViewportSlice(
  ...
  switch (direction) {
    case LoadMoreDirection.Backwards:
      ...
      break;
    case LoadMoreDirection.Forwards:
      areSomeLocal = offsetId !== undefined ? indexForDirection < length : Boolean(length);
      areAllLocal = offsetId !== undefined ? to <= length - 1 : newViewportIds.length > 0;
      break;
    case LoadMoreDirection.Around:
    default:
