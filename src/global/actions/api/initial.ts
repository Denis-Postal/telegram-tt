...
import { clearStoredBotToken, loadStoredBotMode, loadStoredBotToken, storeBotToken } from '../../../api/botApi';
import { clearBotCacheFromStorage, loadBotCacheFromStorage, saveBotCacheToStorage } from '../../../api/botApi/cache';
import { consumePendingBotAuthToken, setPendingBotAuthToken } from '../../../api/botApi/pendingInit';
import {
  callApi, callApiLocal, initApi, setShouldEnableDebugLog,
} from '../../../api/gramjs';
import { resetApiReady } from '../../../api/gramjs/helpers/apiReady';
import {
...
addActionHandler('initApi', (global, actions): ActionReturnType => {
  ...
  const botAuthToken = consumePendingBotAuthToken() || loadStoredBotToken();
  const isBotMode = Boolean(botAuthToken) || loadStoredBotMode();
  const botCache = isBotMode ? loadBotCacheFromStorage() : undefined;
  ...
  void initApi(actions.apiUpdate, {
    ...
    sessionData: isBotMode ? undefined : loadStoredSession(),
...
addActionHandler('goToAuthBotToken', (global): ActionReturnType => {
  return updateAuth(global, {
    state: 'authorizationStateWaitBotToken',
    isLoading: false,
    errorKey: undefined,
  });
});

addActionHandler('loginWithBotToken', async (global, actions, payload): Promise<void> => {
  const { token } = payload;
  ...
  storeBotToken(token);
  setPendingBotAuthToken(token);
  clearStoredSession();
  clearBotCacheFromStorage();
