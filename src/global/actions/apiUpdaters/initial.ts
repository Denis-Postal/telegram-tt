...
import { markApiReady } from '../../../api/gramjs/helpers/apiReady';
import { startBotPollingIfNeeded, stopBotPolling } from '../api/botPolling';
import { clearBotCacheFromStorage, saveBotCacheToStorage } from '../../../api/botApi/cache';
import {
...
addActionHandler('apiUpdate', (global, actions, update): ActionReturnType => {
  switch (update['@type']) {
    ...
    case 'updateCurrentUser':
      ...
      startBotPollingIfNeeded(getGlobal());
      break;
    ...
    case 'saveBotCache':
      saveBotCacheToStorage(update.snapshot);
      break;
...
function onUpdateApiReady<T extends GlobalState>(global: T) {
  ...
  markApiReady();
  startBotPollingIfNeeded(global);

  global = getGlobal();
  const { sync } = getActions();
