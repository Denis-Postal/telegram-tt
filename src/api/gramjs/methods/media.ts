...
import { downloadBotMedia, isBotFileUrl, isBotSession } from '../../botApi';
...
async function download(
  ...
  if (isBotSession()) {
    const botAvatarMatch = url.match(/^(?:avatar|profile)([-\d]+)\?/);
    if (botAvatarMatch) {
      return undefined;
    }
  }
  ...
  if (isBotFileUrl(url)) {
    const result = await downloadBotMedia({ url, mediaFormat: ApiMediaFormat.BlobUrl });
    if (!result) return undefined;
    return {
      mimeType: result.mimeType,
      data: result.dataBlob,
      fullSize: result.fullSize,
    };
  }

  const botAvatarMatch = url.match(/^(?:avatar|profile)([-\d]+)\?(botfile:.+)$/);
  if (botAvatarMatch) {
    if (isBotSession()) {
      return undefined;
    }

    const result = await downloadBotMedia({ url: botAvatarMatch[2], mediaFormat: ApiMediaFormat.BlobUrl });
    if (!result) return undefined;
    return {
      mimeType: result.mimeType,
      data: result.dataBlob,
