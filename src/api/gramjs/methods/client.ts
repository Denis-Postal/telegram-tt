...
import { isBotBlockedMethod } from '../../botApi/botInvokeGuard';
import { getBotStubResponse } from '../../botApi/botInvokeStubs';
import {
  destroyBotApi,
  ensureBotSession,
  getBotToken,
...
let isBotModeHint = false;

function shouldUseBotInvokeGuard() {
  return ensureBotSession() || isBotModeHint;
}

function handleBlockedBotInvoke<T extends GramJs.AnyRequest>(request: T) {
  if (DEBUG) {
    log('INVOKE SKIP (bot)', request.className);
  }

  return getBotStubResponse(request) as T['__response'];
}

function markBotSessionFromRpcError() {
  initBotSession();
  isBotModeHint = true;
}
...
export async function init(initialArgs: ApiInitialArgs, onConnected?: NoneToVoidFunction) {
  ...
  const {
    ...
    hasPasskeySupport, botAuthToken: initialBotAuthToken, isBotMode: initialIsBotMode,
  } = initialArgs;

  const botAuthToken = initialBotAuthToken;

  isBotModeHint = Boolean(botAuthToken || initialIsBotMode);

  if (botAuthToken || initialIsBotMode) {
    initBotSession(botAuthToken);
  }
  ...
  try {
    ...
    if (!isBotSession()) {
      await detectBotSession();
    }

    const activeBotToken = botAuthToken || getBotToken();
    if (isBotSession() && activeBotToken) {
      try {
        await initBotApi(activeBotToken, initialArgs.botCache);
        const botUser = getBotUser();
        if (botUser) {
          setMessageBuilderCurrentUserId(botUser.id);
    ...
    initUpdatesManager(invokeRequest, isBotSession());
...
export async function destroy(noLogOut = false, noClearLocalDb = false) {
  if (isBotSession()) {
    destroyBotApi(!noLogOut);
  }

  isBotModeHint = false;

  if (!noLogOut && client.isConnected() && !isBotSession()) {
    await Promise.race([
...
export async function invokeRequest<T extends GramJs.AnyRequest>(
  ...
  if (shouldUseBotInvokeGuard() && isBotBlockedMethod(request.className)) {
    return handleBlockedBotInvoke(request);
  }
  ...
  try {
    ...
    if (message === 'BOT_METHOD_INVALID') {
      markBotSessionFromRpcError();
      if (isBotBlockedMethod(request.className)) {
        return handleBlockedBotInvoke(request);
      }
      return undefined;
    }

    if (shouldUseBotInvokeGuard() && message === 'AUTH_KEY_UNREGISTERED') {
      if (isBotBlockedMethod(request.className)) {
        return handleBlockedBotInvoke(request);
      }
      return undefined;
...
async function detectBotSession() {
  if (isBotSession()) {
    return;
  }
