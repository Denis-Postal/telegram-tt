...
import { botFetchMessages, botSendMessage, isBotSession } from '../../botApi';
import { getBotStoreMessages } from '../../botApi/store';
...
export async function fetchMessage({ chat, messageId }: { chat: ApiChat; messageId: number }) {
  if (isBotSession()) {
    const { messages } = getBotStoreMessages(chat.id, Number.MAX_SAFE_INTEGER);
...
export async function fetchMessagesById({ chat, messageIds }: { chat: ApiChat; messageIds: number[] }) {
  if (isBotSession()) {
    const { messages } = getBotStoreMessages(chat.id, messageIds.length);
...
export async function sendMessage(
  ...
  if (isBotSession()) {
    ...
    const apiMessage = await botSendMessage({ ...params, localMessage });
    if (!apiMessage) {
      return localMessage;
    }
