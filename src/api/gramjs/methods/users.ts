...
export async function fetchFullUser({
  ...
  if (isBotSession()) {
    return botFetchFullUser({ id, accessHash });
  }
...
export async function fetchProfilePhotos({
  ...
  if (isBotSession()) {
    return botFetchProfilePhotos({ peer, offset, limit });
  }
