...
import type { ApiInitialArgs, ApiOnProgress, ApiUpdate } from '../../types';
import type { OriginMessageEvent, WorkerPayload } from './types';

import { initBotSession } from '../../botApi';
  ...
  data.payloads.forEach(async (payload) => {
    switch (payload.type) {
      case 'initApi': {
        ...
        const initialArgs = args[0] as ApiInitialArgs;

        if (initialArgs.botAuthToken || initialArgs.isBotMode) {
          initBotSession(initialArgs.botAuthToken);
        }

        initApi(onUpdate, initialArgs, args[1]).then(() => {
          if (messageId) {
            sendToOrigin({
              type: 'methodResponse',
              messageId,
